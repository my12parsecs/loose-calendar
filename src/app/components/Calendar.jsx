      ...
      <div className="calendar__content">
        <div className={"calendar__items-list"}>
          ...
          {daysListGenerator.prevMonthDays.map((el, index) => {
            ...
            return (
              <div className={`calendar-day ${cleanThisWeek.includes(elDate) ? "calendar-this-week-day" : ""}`} key={`prev-${el}/${index}`}>
                <button className={`calendar__item gray ${isToday(elDate) ? "today-indicator" : ""}`} onClick={() => handleDayClick(elDate)}>
                  {el}
                  <div className={isDayToday(el, prevMonth) ? "today-dot" : ""}></div>
                </button>
              </div>
            );
          })}
          ...
          {daysListGenerator.days.map((el, index) => {
            ...
            return (
              <div key={`current-${index}-/-${el}`} className={`calendar-day ${cleanThisWeek.includes(elDate) ? "calendar-this-week-day" : ""}`}>
                <button className={`calendar__item ${isToday(elDate) ? "today-indicator" : ""}`} onClick={() => handleDayClick(elDate)}>
                  <div className="day__layout">
                    <div className="text">{el.toString()}</div>
                  </div>
                  <div className={isDayToday(el, thisMonth) ? "today-dot" : ""}></div>
                </button>
              </div>
            );
          })}
          ...
          {daysListGenerator.remainingDays.map((el, idx) => {
            ...
            return (
              <div className={`calendar-day ${cleanThisWeek.includes(elDate) ? "calendar-this-week-day" : ""}`} key={`next-${idx}----${el}`}>
                <button className={`calendar__item gray ${isToday(elDate) ? "today-indicator" : ""}`} onClick={() => handleDayClick(elDate)}>
                  {el}
                  <div className={isDayToday(el, nextMonth) ? "today-dot" : ""}></div>
                </button>
              </div>
            );
          })}
